...
<section class="w-100 bg-white rounded overflow-hidden shadow">
    ...
    <div class="py-2.5 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2"
        ...
        <h3 class="text-md m-0">Header Sliders</h3>
        <a href="{{ route('header-sliders.create') }}" class="btn btn-primary btn-sm ms-2"
                ...
                @forelse($sliders as $slider)
                    <tr class="text-center">
                        ...
                        <td class="d-none d-sm-table-cell py-1 ">
                            <form action="{{ route('header-sliders.headerToggleStatus', $slider->id) }}" method="POST">
                                @csrf
                        ...
                        <td>
                            <div class="d-flex gap-2 justify-center">
                                <a href="{{ route('header-sliders.edit', $slider->id) }}" 
                                   class="flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-medium bg-blue-500 text-white hover:bg-blue-600 shadow-md transition">Edit</a>
                                <form action="{{ route('header-sliders.destroy', $slider->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-medium bg-red-500 text-white hover:bg-red-600 shadow-md transition"
