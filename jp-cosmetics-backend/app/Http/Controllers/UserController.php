...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $cloudImageUploadResponse = $this->fileStorageService->uploadImageToCloud($image, 'users');
            $superAdmin->image = $cloudImageUploadResponse['public_path'];
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($superAdmin->image) {
                ...
                $fileToDelete = $superAdmin->image;
                $imageUploadResponse = $this->fileStorageService->updateFileFromCloud($fileToDelete, $newFile);
                $superAdmin->image = $imageUploadResponse['public_path'];
            } else {
                $image = $request->file('image');
                $imageUploadResponse = $this->fileStorageService->uploadImageToCloud($image, 'user');
                $superAdmin->image = $imageUploadResponse['public_path'];
            }
        }
